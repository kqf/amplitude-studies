...
def scan(n=4, ntrials=10):
    ...
    algorithms = ('Nelder-Mead', 'Powell', 'CG', 'BFGS',
                  'L-BFGS-B', 'TNC', 'SLSQP', MinuitMethod())

    results = [check(data, a) for a in algorithms]
    mean_iterations = {a: i for a, (i, e) in zip(
        algorithms, results) if i > 0.0}

    for a, niter in mean_iterations.iteritems():
        string = a, niter, n
        msg = "The algorithm {0} requires {1} iterations for {2} parameters" 
        print msg.format(*string)
